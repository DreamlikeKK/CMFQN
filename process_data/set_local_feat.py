...
if __name__ == "__main__":
    ...
    for boolean_value in bool_list:
        IS_TRAIN = boolean_value

        save_dir = "/home/shunlizhang/zwk/CMFQN/data/weibo_dataset_local"
        ...
        if IS_TRAIN:
            train_dataset = weibo_dataset(is_train=True)
            data = DataLoader(train_dataset, batch_size=1, shuffle=False,
                                collate_fn=collate_fn_chinese,
                                num_workers=4, sampler=None, drop_last=True,
                                pin_memory=True)
        else:
            validate_dataset = weibo_dataset(is_train=False)
            data = DataLoader(validate_dataset, batch_size=1, shuffle=False,
                                        collate_fn=collate_fn_chinese,
